...
class BOMHybridModel:
    ...
    def convert_data_types(self):
        ...
        for col in month_cols:
            self.df_usage[col] = pd.to_numeric(self.df_usage[col], errors='coerce')
            self.df_purchase[col] = pd.to_numeric(self.df_purchase[col], errors='coerce')
        
        # 재고 데이터 변환
        for col in self.df_inventory.columns:
            if col not in ['품목코드', '품목명']:
                self.df_inventory[col] = pd.to_numeric(self.df_inventory[col], errors='coerce')
                    ...
                    for col in self.available_months:
                        if col in self.df_brand.columns:
                            try:
                                ...
                                ratios.append(float(val) if val else default_ratios[brand])
                            except:
    ...
    def get_inventory(self, material_code):
        ...
        try:
            # 원료코드를 문자열로 변환하여 비교
            material_code_str = str(material_code)
            
            # 품목코드 컬럼에서 매칭
            for idx, row in self.df_inventory.iterrows():
                if str(row['품목코드']) == material_code_str:
                    # 마지막 유효한 재고값 찾기
